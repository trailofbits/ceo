...
from tools import aflshowmap_path
...
def afl_showmap(input_path, args, first=False):
    cmd = [
            aflshowmap_path,
...
class Worker(multiprocessing.Process):
    ...
    def run(self):
        ...
        while True:
            ...
            idx, args, input_path = job
            r = afl_showmap(input_path, args)
            counter.update(r)
...
def cmin(exe, input_dir, output_dir, debug=0):
    ...
    args.no_dedup = True
    args.edge_mode = False
    args.args = []
    args.stdin_file = False
    args.workers = 1
    args.as_queue = False
    ...
    #print ('Obtaining trace results')
    for i,f in enumerate(files):
        job_queue.put((i, args, f))
    for _ in range(args.workers):
        job_queue.put(None)
    job_queue.close()
    ...
    #print ('Processing candidates and writing output')
    already_have = set()
    count = 0
