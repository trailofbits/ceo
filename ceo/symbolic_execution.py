...
def make_initial_symbolic_state(binary_path, concrete_data):
    assert(0)
    magic = file(binary_path).read(4)
    if magic == '\x7fELF':
        # Linux
        state = make_symbolic_linux(binary_path, concrete_data)
    elif magic == '\x7fCGC':
        # Decree
        state = make_symbolic_decree(binary_path, concrete_data)
    elif magic == '#EVM':
        assert(0)
...
def make_symbolic_decree(program, concrete_data, **kwargs):
