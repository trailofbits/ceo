...
def explore(options, tc):
    ...
    if "grr" in options:
        ...
        label = explore.run(verbose=1, timeout=600)
        explore.save_results(input_dir, crash_dir)
        labels.append(("grr", label))

    if "afl" in options:
        explore = ExploreAFL( tc.target_filepath, tc.input_filepath, tc.target_filename+"/afl")
        param_features.append(("afl", explore.get_features()))
        label = explore.run(verbose=0, timeout=600)
